...
public class YoutubeOauth2HandlerOG {
    private static final Logger log = LoggerFactory.getLogger(YoutubeOauth2HandlerOG.class);
    ...
    public YoutubeOauth2HandlerOG(HttpInterfaceManager httpInterfaceManager) {
     ...
     * be used instead.
     */
    public String initializeAccessToken(@Nullable CommandSourceStack commandSourceStack) {
        ...
        new Thread(() -> pollForToken(deviceCode, interval == 0 ? 5000 : interval, commandSourceStack), "youtube-source-token-poller").start();
        return userCode;
    }
    ...
    private void pollForToken(String deviceCode, long interval, CommandSourceStack commandSourceStack) {
        // @formatter:off
        String requestJson = JsonWriter.string()
            .object()
                .value("client_id", CLIENT_ID)
        ...
        while (true) {
            try (HttpInterface httpInterface = getHttpInterface();
                ...
                DisxSystemMessages.refreshTokenGenerated(commandSourceStack);
                DisxConfigHandler.SERVER.updateProperty("refresh_token", refreshToken);
                enabled = true;
                return;
